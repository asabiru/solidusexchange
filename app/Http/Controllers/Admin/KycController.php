...
class KycController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->provider === 'manual') {
            $rules['field_name.*'] = 'required|string';
            $rules['input_type.*'] = 'required|in:text,textarea,file,date,number';
            $rules['is_required.*'] = 'required|in:required,optional';
        }
        ...
        try {
            $inputForm = [];
            if ($request->provider === 'manual' && $request->has('field_name')) {
