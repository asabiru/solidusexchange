...
class LogoController extends Controller
{
    ...
    public function logoUpdate(Request $request)
    {
        ...
        $request->validate([
            ...
            'dark_logo' => 'sometimes|required|mimes:jpg,png,jpeg|max:2048',
        ...
        if ($request->hasFile('dark_logo')) {
            try {
                $image = $this->fileUpload($request->dark_logo, config('filelocation.logo.path'), null, null,'webp', 60, $basicControl->dark_logo, $basicControl->dark_logo_driver);
                if ($image) {
                    $basicControl->dark_logo = $image['path'];
