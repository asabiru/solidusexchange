    ...
    <script>
        ...
        if (telegramLoginWidget && window.matchMedia('(min-width: 768px)').matches) {
            const renderTelegramWidget = function () {
                ...
                // Fail-safe: if the script is blocked or takes > 1.5s to load, trigger the fallback instantly
                const scriptLoadTimeout = window.setTimeout(function () {
                    if (widgetScript.parentNode) {
                        console.warn('Telegram widget loading timed out. Showing fallback button.');
