                        ...
                        <div class="card mb-3 mb-lg-5">
                            ...
                            <div class="card-body">
                                <ul class="list-unstyled list-py-2 text-dark mb-0">
                                    ...
                                    <li><i class="bi-phone dropdown-item-icon"></i>{{ $user->phone_code }} {{ $user->phone }}</li>
                                </ul>
                            </div>
                        </div>
                                    ...
                                    <table
                                        ...
                                        @forelse($transactions as $key =>  $transaction)
                                            <tr>
                                                ...
                                                <td>
                                                    @if($transaction->transactional_type == \App\Models\BuyRequest::class)
                                                        <h6 class="mb-0 {{ $transaction->trx_type == '+' ? 'text-success' : 'text-danger' }}">{{ $transaction->trx_type . currencyPosition($transaction->amount) }}| <sup class='text-dark'>{{number_format($transaction->transaction_amount, 2)}} {{$transaction->transaction_currency}}</sup></h6>
                                                    @else
                                                        <h6 class="mb-0 {{ $transaction->trx_type == '+' ? 'text-success' : 'text-danger' }}">{{ $transaction->trx_type . currencyPosition($transaction->amount) }}| <sup class='text-dark'>{{rtrim(rtrim($transaction->transaction_amount, 0), '.')}} {{$transaction->transaction_currency}}</sup></h6>
                                                    @endif
                                                </td>
                                                <td> {{currencyPosition($transaction->charge)}}</td>
                                    ...
                                    <table
                                        ...
                                        @forelse($paymentLog as $key =>  $payment)
                                            <tr>
                                                ...
                                                <td>
                                                    <h6 class="mb-0 {{ $payment->getStatusClass() }}">{{currencyPosition(number_format($payment->payable_amount_base_currency,2))}}  </h6>
                                                </td>
                                                <td>
                                                    @if ($payment->status == 1)
                                                        <span
